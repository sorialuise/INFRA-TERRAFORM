resource "aws_autoscaling_group" "asgcadmin" {
  name                      = var.name_asg
...
resource "aws_cloudwatch_metric_alarm" "ec2-up" {
  ...
  dimensions = {
    AutoScalingGroupName = aws_autoscaling_group.asgcadmin.name
  }
...
resource "aws_autoscaling_notification" "notifications_asg" {
  group_names = [aws_autoscaling_group.asgcadmin.name]
