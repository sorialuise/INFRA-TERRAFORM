...
wget https://nginx.org/keys/nginx_signing.key  >> /tmp/log.txt 2>&1
apt-key add nginx_signing.key     >> /tmp/log.txt 2>&1
apt-get update     >> /tmp/log.txt 2>&1
apt-get install nginx-module-xslt -y  >> /tmp/log.txt 2>&1
...
server {
  ...
  server_name qa.perucheck.pe;

   access_log  /var/log/nginx/qa.perucheck.pe-access.log main;
   error_log   /var/log/nginx/qa.perucheck.pe-error.log info;
...
" >> /etc/nginx/conf.d/qa.perucheck.conf
...
# renombrar vhost "default"
mv /etc/nginx/conf.d/default /etc/nginx/conf.d/bk-default >> /tmp/log.txt 2>&1
...
sed -i s%"listen 80;"%"listen 8080;"%g /etc/nginx/conf.d/qa.perucheck.conf  >> /tmp/log.txt 2>&1
